...
const RoomCard = ({
  ...
  return (
    <div className="col-span-1 border flex flex-col border-blue-600 rounded-md">
      <div className="py-4 text-center rounded-t-md font-semibold bg-blue-600 text-white px-6">
        {title}
      </div>
      <div className="flex flex-col flex-auto gap-2 p-4 items-center justify-between">
        <div className="flex flex-col gap-2">
          ...
          <span>
            ⚡ Giá điện:{" "}
            <span className="font-bold text-orange-600">
              {formatMoney(+electricPrice) + " VNĐ/kWh"}
            </span>
          </span>
          <span>
            💧 Giá nước:{" "}
          ...
          <span>
            ...
            <span className="font-bold text-orange-600">
              {formatMoney(+capsPrice) + " VNĐ/tháng"}
            </span>
          </span>
          <span>
            🌐 Giá internet:{" "}
            <span className="font-bold text-orange-600">
              {formatMoney(+internetPrice) + " VNĐ/tháng"}
            </span>
          </span>
          <span>
            🔲 Diện tích:{" "}
          ...
          <span>
            👨‍👩‍👧‍👧 Số người ở tối đa:{" "}
            <span className="font-bold text-orange-600">{stayMax}</span>
          </span>
          <span>
            ✅ Trạng thái:{" "}
            <span className="font-bold text-orange-600">{position}</span>
          </span>
          <div className="flex flex-col mt-4 gap-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...new Set(
                  rConvenients?.map((c) =>
                    JSON.stringify({ rValues: c.rValues })
                  )
                ),
              ]
